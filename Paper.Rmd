...
The goal of this section is not "confirmatory" whatsoever. Both dependent and independent variables are going to be introduced with an emphasis on their quantitative features such as their distributions and potential transformations which can be applied to the variables in question. In addition to this, bivariate relations between independent and dependent variables will be plotted in order to get an initial idea about the potential effects along with similar findings from the literature. Thus, in a sense, EDA in this section doesn't carry out the purpose of discovery, rather it merely puts the data on the spot for the sake of transparency.
...
"Total Votes" is going to be used as a control variable since 80% Perceived Helpfulness can arise from both 4 Helpful votes over 5 Total votes or 80 Helpful votes over 100 Total votes [@mudambi2010; @liu2014; @choi2020].
...
-   talk about review and reviewer related determinant, search/exp classification is too simple,

# Conclusion

This study collected Online Customer Reviews data from two different sources (IMDB and BestBuy) and analyzed the data within the context of consumer uncertainty. Negativity Bias Theorem, Prospect Theory and Search / Experience goods paradigm from information economics were used as an theoretical engine to explain the information gathering process of a consumer. Results showed similarities from the existing Review Helpfulness literature. However, once again it should be noted that the literature has many conflicting results.
...
There are some implications for future studies. Firstly, aside from lexicon based sentiment analysis it is also possible to utilize deep learning methods to mine opinions from a text. One advantage of deep learning method over a lexicon is its versatility. A negative word can change from one context to another and training a deep learning method is more robust to this type of contextual changes whereas with lexicons a negative word is a negative word no matter the context is. Secondly, future studies should consider collecting data from different regions and cultures. There might be some differences on how consumers process and react to the information in order to deal with uncertainty. Lastly, researchers can dig deeper into search and experience goods paradigm. It is well established in the literature that product type affects the information gathering process. However, there might be differences within product types depending on their search and experience attributes. An objective way to identify and index experience and search attributes may also shed some light to the conflicting results in the literature.
