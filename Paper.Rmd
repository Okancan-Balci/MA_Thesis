...
The existence of the Negativity Bias and how the potential effects of Negativity Bias on Review Helpfulness could be moderated by either Search And Experience goods will be the main focus of this study. A lesser degree of Negativity Bias is expected for Experience Goods. Additionally, the amount of information that a review has and its connection to Review Helpfulness with respect to Review Ratings will be analyzed. Next, the diagnostic values of the negative and positive sentiments are going to be discussed. Because of the Negativity Bias, a positive relationship between Negative Sentiments and Review Helpfulness is expected. Lastly, the positive relationship between the Review Helpfulness and the timeline of a review will be inquired because an updated and new information might be more helpful than an old information.
...
Negativity Bias argues that humans or even animals have tendencies to be more aware of negative entities than positive ones. In @rozin2001's words Negativity Bias can be best exemplified such as the following:
...
@rozin2001 explains the Negativity Bias by describing it with four main aspects. Namely, Negative Potency, Steeper Negative Gradients, Negative Dominance and Negative Differentiation [@rozin2001; @sen2007; @eslami2018]. Loss Aversion has a strong connection to Negativity Bias since negative cues might be used as an early warning against potential losses [@herr1991].
...
p3 <- search_goods %>% 
  mutate(negative = 10^(negative)) %>% 
  ggplot(aes(negative))+
  geom_histogram(color = "white", fill = "steelblue", boundary = 0)+
  geom_vline(aes(xintercept = mean(negative)), size = 1, alpha = 0.6)+
  annotate(geom = "label", x = 10^mean(search_goods$negative), y = Inf, 
           label = paste("Mean:", round(10^mean(search_goods$negative), digits = 2)),
           vjust = 1, hjust = 0, size = 4.5)+
  scale_x_continuous()+
  labs(y = "Count", x = "Proportion of Negative Words")
...
p4 <- search_goods %>% 
  ...
  labs(y = "Helpfulness", x = "Proportion of Negative Words")
...
```{r Experience-Neg, fig.height=6, fig.cap="The Distribution(A) and the Scatter Plot(C) of Proportion of Negative Words for Experience Goods and their respective logarithmic transformations (B), (D)"}
p1 <- movies_data %>% 
  ggplot(aes(negative))+
  geom_histogram(color = "white", fill = "steelblue", boundary = 0)+
  geom_vline(aes(xintercept = mean(negative)), size = 1, alpha = 0.6)+
...
p3 <- movies_data %>% 
  ...
  geom_histogram(color = "white", fill = "steelblue", boundary = 0)+
  geom_vline(aes(xintercept = mean(negative)), size = 1, alpha = 0.6)+
  annotate(geom = "label", x = 10^mean(movies_data$negative), y = Inf, 
           label = paste("Mean:", round(10^mean(movies_data$negative), digits = 2)),
           vjust = 1, hjust = 0, size = 4.5)+
  scale_x_continuous()+
  labs(y = "Count", x = "Proportion of Negative Words")
...
p3 <- search_goods %>% 
  mutate(positive = 10^positive) %>% 
  ggplot(aes(positive))+
  geom_histogram(color = "white", fill = "steelblue", boundary = 0)+
  geom_vline(aes(xintercept = mean(positive)), size = 1, alpha = 0.6)+
  annotate(geom = "label", x = 10^mean(search_goods$positive), y = Inf, 
           label = paste("Mean:", round(10^mean(search_goods$positive), digits = 2)),
           vjust = 1, hjust = 0, size = 4.5)+
  scale_x_continuous()+
  labs(y = "Count", x = "Proportion of Positive Words")
...
p4 <- search_goods %>% 
  ...
  labs(y = "Helpfulness", x = "Proportion of Positive Words")
...
```{r Experience-Pos, fig.height=6, fig.cap="The Distribution(A) and the Scatter Plot(C) of Proportion of Positive Words for Experience Goods and their respective logarithmic transformations (B),(D)"}
p1 <- movies_data %>% 
  ggplot(aes(positive))+
  geom_histogram(color = "white", fill = "steelblue", boundary = 0)+
  geom_vline(aes(xintercept = mean(positive)), size = 1, alpha = 0.6)+
...
p3 <- movies_data %>% 
  ...
  geom_histogram(color = "white", fill = "steelblue", boundary = 0)+
  geom_vline(aes(xintercept = mean(positive)), size = 1, alpha = 0.6)+
  annotate(geom = "label", x = 10^mean(movies_data$positive), y = Inf, 
           label = paste("Mean:", round(10^mean(movies_data$positive), digits = 2)),
           vjust = 1, hjust = 0, size = 4.5)+
  scale_x_continuous()+
  labs(y = "Count", x = "Proportion of Positive Words")
...
Readers may be curious about the existence of negation words such as "not", "no", etc.. Additionally, reviewers might also be sarcastic when writing their reviews and they might say thing like "This product was so good that I could not believe it!". @Tidytext discusses that both negation and sarcasm were used in a lesser degree in the English language. Therefore, given the enough sample size non-negated words and non-sarcastic use of the language get dominated by the normal use of the words.
...
As it was mentioned in Section \@ref(negativity-bias-and-loss-aversion), people tend to try to minimize their losses. Because of this, negative cues draw more attention than positive ones since negative cues may imply potential losses. @guo2019 found out that people spent more time when evaluating negative reviews. This may suggest a linear interaction between Review Extremity and Review Length. Since people take more time to evaluate negative reviews, this may indicate that the effect of Review Length is stronger when Review Rating is low. And when a review is longer it is perceived as more helpful for negatively rated reviews than positively rated reviews. In this case, it is hypothesized that:
...
@kuan2015 discussed that since the majority of the reviews are positive these positive reviews lose their diagnostic values for a given product. Compared to negative information which may lay out potential risks for the product in a much more objective sense, positive information might be found more subjective and lay out a more ambiguous information about the product which may confuse potential consumers. Therefore, it is hypothesized that:
...
Tobit regression results are presented at Table \@ref(tab:Reg-Results-Tobit) alongside with OLS results as a benchmark at Table \@ref(tab:Reg-Results-OLS) [@mudambi2010; @kuan2015; @choi2020]. Both Tobit regression results and OLS results have the same signs and significances which might indicate that the results are somewhat robust. For Search goods, the magnitudes of the coefficients are usually higher . This is quite expected since Search good data has more censored observations compared to Experience goods data.
...
Online Consumer Reviews data were collected from two different sources (IMDB and BestBuy) and analyzed within the context of consumer uncertainty in this study. Negativity Bias, Prospect Theory and Search / Experience goods paradigm from information economics were used as theoretical engines to explain the information gathering process of an average consumer. Results showed similarities from the existing Review Helpfulness literature. However, once again it should be noted that the literature has many conflicting results.

This study contributes to the literature as an additional empirical work with a unique BestBuy review data and a unique combination of movie reviews from IMDB. In addition, to my knowledge, this is the first study which analyzed the linear continuous interaction between the Review Length and the Review Rating in terms of Negativity Bias. Additionally, there is a lack of emphasis on the sentiment analysis in the existing literature. By employing positive and negative sentiments in terms of Negativity Bias and Prospect Theory this study shows the viability of the sentiment analysis in this context.
